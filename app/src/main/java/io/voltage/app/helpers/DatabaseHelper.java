...
public interface DatabaseHelper {
    ...
    class Default implements DatabaseHelper {
        ...
        public Registration getRegistration(final Context context) {
            final Uri uri = VoltageContentProvider.Uris.REGISTRATIONS;

            final ContentResolver resolver = context.getContentResolver();
            final Cursor cursor = resolver.query(uri, null, null, null, null);

            try {
                return DataUtils.getList(cursor, Registration.class).get(0);
        ...
        public void updateThread(final Context context, final String threadId, final String threadName) {
            try {
                final ArrayList<ContentProviderOperation> operations = new ArrayList<>();
                operations.add(mOperationHelper.updateThreadOperation(threadId, threadName));
